...
export default async function ShopAdminPage() {
  try {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-12 bg-neutral-900 text-white">
          <div className="max-w-2xl w-full">
              <h1 className="text-4xl font-black text-red-500 mb-8 flex items-center gap-4">
                  <AlertTriangle className="w-10 h-10" /> FATAL CRASH
              </h1>
              <div className="bg-neutral-800 p-8 rounded-[40px] border border-neutral-700 shadow-2xl">
                  <p className="text-sm font-bold uppercase tracking-widest text-neutral-500 mb-2">Error Message</p>
                  <pre className="text-red-400 font-mono text-sm whitespace-pre-wrap mb-8 bg-black/30 p-4 rounded-2xl">{err.message}</pre>
                  
                  <p className="text-sm font-bold uppercase tracking-widest text-neutral-500 mb-2">Stack Trace</p>
                  <pre className="text-neutral-400 font-mono text-[10px] whitespace-pre-wrap overflow-auto max-h-[400px] bg-black/30 p-4 rounded-2xl">{err.stack}</pre>
              </div>
              <button onClick={() => window.location.reload()} className="mt-8 px-8 py-4 bg-white text-black rounded-full font-bold hover:bg-neutral-200 transition-all">
                  Reload System
              </button>
          </div>
      </div>
